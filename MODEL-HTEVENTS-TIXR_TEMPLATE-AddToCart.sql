-- TIXR: ADD TO CART HT EVENTS MODEL --
...
UUIDRanked AS (
    ...
    COALESCE(p.FBC, pr.FBC) as FBC,

    -- ADD TO CART INFORMATION
    '[' ||
    '''' || REGEXP_SUBSTR(p.PRODUCT_ID, '[0-9]+', 1, 1) || '''' ||
    ']' AS CONTENT_IDS,
    '[{' ||
    'id: ''' || REGEXP_SUBSTR(p.PRODUCT_ID, '[0-9]+', 1, 1) || ''',' ||
    ' quantity: ' || p.QUANTITY ||
    '}]' AS CONTENTS,
    ...
    COALESCE(p.FBP, i.FBP, o.FBP, pr.FBP) as FBP,
...
FROM <<SCHEMA_NAME_GOES_HERE>>.ADD_TO_CART p
