-- TIXR: INITIATE CHECKOUT HT EVENTS MODEL --

WITH CheckoutWithParsed AS (
  SELECT *,
    CASE 
  ...
  FROM <<SCHEMA_NAME_GOES_HERE>>.INITIATE_CHECKOUT
),
IdentifiesRanked AS (
...
WHERE rn = 1
GROUP BY
    UUID, TIMESTAMP, CONTEXT_PAGE_URL, CONTEXT_PAGE_PATH, CONTEXT_PAGE_TITLE,
    CONTEXT_PAGE_SEARCH, CONTEXT_PAGE_REFERRER, FBC, CONTENT_TYPE, QUANTITY, VALUE, CURRENCY,
    EXTERNAL_ID, EMAIL, PHONE_NUMBER, FIRST_NAME, FIRST_INITIAL, LAST_NAME, BIRTHDAY,
    GENDER,ZIP_CODE, FIRST_NAME, FIRST_INITIAL, LAST_NAME, FBP, IP_ADDRESS,
